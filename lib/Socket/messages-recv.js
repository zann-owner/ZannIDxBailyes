...
import { extractGroupMetadata, resolveGroupParticipantsLid } from './groups.js';
...
export const makeMessagesRecvSocket = (config) => {
    ...
    const handleGroupNotification = async (fullNode, child, msg) => {
        ...
        switch (child?.tag) {
            case 'create':
                const metadata = await extractGroupMetadata(child, signalRepository.lidMapping);
            ...
            case 'leave':
                ...
                const rawParticipants = getBinaryNodeChildren(child, 'participant').map(({ attrs }) => ({
                    jid: attrs.jid,
                ...
                // Resolve LID -> PN lewat helper bersama yang sama dengan extractGroupMetadata().
                const participants = await resolveGroupParticipantsLid(rawParticipants, signalRepository.lidMapping);
    ...
    console.log("PARTICIPANT:", node.attrs.participant)
    console.log("PARTICIPANT_PN:", node.attrs.participant_pn)
    console.log("SENDER_PN:", node.attrs.sender_pn)
    console.log("PEER_RECIPIENT_PN:", node.attrs.peer_recipient_pn)
    console.log("========================")
